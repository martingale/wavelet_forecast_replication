...
forecMain <- function(perData, tstep, models,step, tau){
  tstep <- tau - tstep
  ...
  forecs <- lapply(models, function(m) sapply(1:lp, function(p) sapply(wav_all[[p]], 
                  function(w)  tryCatch(ForecastF(w,m,step,theargs,MLT=1,no=p, tau=tau),
                  error= function(e)  tryCatch(ForecastF(w,m,step,theargs,MLT=1e3, no=p, tau=tau),
                  error= function(e)  tryCatch(ForecastF(w,m,step,theargs, MLT=1e6, no=p, tau=tau)))))))
...
forecAnalysis <- function(tmFreq,interval,type,tau,step){
  ...
  declMax <- 8 # maximum decomposition level
  # browser()
  myData <- dataMnp(5,type,interval,tau)
  den <- myData[[2]]
  myData <- myData[[1]]
  ...
  models     <- c("garchFit", "nnetTs","msvar")
  # models     <- c( "nnetTs")
  forecsNMG <- mclapply(fromm:tst, function(t) forecMain(perData,t, models, step, tau=tau))
  browser()
  ...
  forecsNMG  <- lapply(1:3, function(m) lapply(1:25, function(w) t(sapply(1:tau, function(t) forecsNMG[[t]][[m]][w,]))))
   ...
   models     <- c("lstar")
  forecsL <- mclapply(fromm:tst, function(t) forecMain(perData,t,models, step, tau=tau))
  #browser()
  ...
  forecsL    <- lapply(1,   function(m) lapply(1:25, function(w) t(sapply(1:tau, function(t) forecsL[[t]][[m]][w,]))))
   ...
   models     <- c("setar")
  forecsS <- mclapply(fromm:tst, function(t) forecMain(perData,t,models, step, tau=tau)
                      ,mc.preschedule = T, mc.allow.recursive = F, mc.cores = 12)
  ...
  forecsS    <- lapply(1,   function(m) lapply(1:25, function(w) t(sapply(1:tau, function(t) forecsS[[t]][[m]][w,]))))
   ...
   models     <- c("ar","arma")
  forecsARMA <- mclapply(fromm:tst, function(t) forecMain(perData,t,models, step,tau = tau))
  # browser()
...
\\begin{document}"
  ...
  cat(doc.start,file=paste0(filName,"/resultsAll-",type,"-st",step,"_",tau,".tex"),append=T)
  for(i in 1:lp){
    ...
    print.xtable(xtb,file = paste0(filName,"/resultsAll-",type,"-st",step,"_",tau,".tex"),append = T,caption.placement="top",hline.after=c(-1,0,(1+declMax)*1:lm),sanitize.text.function = identity)
  }
  cat("\\end{document}",file=paste0(filName,"/resultsAll-",type,"-st",step,"_",tau,".tex"),append=T)
